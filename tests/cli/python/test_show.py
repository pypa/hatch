...
from .utils import downgrade_distribution_metadata, downgrade_version, write_distribution
...
def test_some_installed(hatch, temp_dir_data, dist_name):
...
def test_all_installed(hatch, temp_dir_data):
    install_dir = temp_dir_data / 'data' / 'pythons'
    compatible_distributions = get_compatible_distributions()
...
def test_specific_location(hatch, temp_dir_data, dist_name):
...
def test_outdated(hatch, temp_dir_data, dist_name):
    install_dir = temp_dir_data / 'data' / 'pythons'
    write_distribution(install_dir, dist_name)
    ...
    installed_table = render_table(
        ...
        [
            ['Name', 'Version', 'Status'],
            [dist_name, downgrade_version(installed_distribution.version.base_version), 'Update available'],
        ],
    )
    available_table = render_table(
        'Available',

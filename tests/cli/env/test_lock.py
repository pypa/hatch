...
def test_uv_in_sync_seeds_existing_lockfile(helpers, monkeypatch, temp_dir):
    """``in_sync`` must pass the committed lock to ``generate`` as a seed.

    ``uv pip compile`` keeps compatible pins when the output file already
    exists; a blank temp file would resolve to the newest versions and make
    ``hatch env lock --check`` fail on every new release (#2406).
    """
    lock_path = temp_dir / "pylock.toml"
    lock_body = helpers.dedent(
        """

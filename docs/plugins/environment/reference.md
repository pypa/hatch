...
For moving artifacts between the workspace and the environment, custom lockers should prefer [`fs_context`](#hatch.env.plugin.interface.EnvironmentInterface.fs_context) and [`FileSystemContext`](../utilities.md#hatch.env.plugin.interface.FileSystemContext) over ad hoc I/O, as discussed in [#355](https://github.com/pypa/hatch/discussions/355).

...
You can specify constraints on the runtime version of Hatch by providing a value for `tool.hatch.requires`. 
...
!!! note
    This field was introduced in version 1.18.0. Earlier versions of Hatch thus ignore the field, so `requires` cannot be used to reject these versions of Hatch.

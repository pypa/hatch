...
class WheelArchive:
    ...
    def write_shared_script(
        ...
        return self.write_file(
            relative_path,
            contents,
            mode=normalize_file_permissions(file_stat.st_mode)
            if self.reproducible
        ...
        with (
            ...
            for included_file in self.recurse_selected_project_files():
                ...
                if len(path_parts) == 1:  # no cov
                    top_module_name = os.path.splitext(relative_path)[0]
                    relative_path2root = os.path.join(self.root, relative_path)
                    exposed_paths4packages[relative_path2root] = top_module_name
                    continue

                # Root package
                root_module = path_parts[0]
                if distribution_path == relative_path:
                    exposed_paths4packages[os.path.join(self.root, root_module)] = (
                        root_module
                    )
                else:
                    distribution_module = distribution_path.split(os.sep)[0]
                    try:
                        relative_path2root = os.path.join(
                            self.root,
                            f"{relative_path[: relative_path.index(distribution_path)]}{distribution_module}",
                        )
                        exposed_paths4packages[relative_path2root] = distribution_module
                    except ValueError:
                        message = (
            ...
            if self.config.dev_mode_exact:
                for relative_path, module in exposed_paths4packages.items():
                    editable_project.map(module, relative_path)
            else:
                for relative_path in exposed_paths4packages:
